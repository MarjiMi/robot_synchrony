...
m0 <- lm(gators_diff ~ condition, data = d1)
summary(m0)
m0.1 <- lm(gators_diff ~ 1, data = d1)
summary(m0.1)

# Filter the data to include only the control group (condition = 0)
control_data <- d1 |>
  filter(condition == 0)
m0.2 <- lm(gators_diff ~ 1, data = control_data)
summary(m0.2)

exp_data <- d1 |>
  filter(condition == 1)
m0.3 <- lm(gators_diff ~ 1, data = exp_data)
summary(m0.3)
